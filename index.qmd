...
- **The Goal:** Maximize population health under a strictly constrained budget.
...
- **The Cost-Effectiveness Plane:** Plotting $\Delta\text{Cost}$ against $\Delta\text{Effect}$. Where does your engineering solution land?
...
# Analytics for Engineering Solutions
...
- PPV is highly dependent on *background prevalence*. 
  - *Takeaway for Engineers:* A highly specific device can still yield mostly false positives if deployed in a low-prevalence population!
...
- **Frequentist vs. Bayesian:** - *Frequentist:* The true rate of AMR is fixed; our data is a random sample.
...
- **The False Positive Paradox:** Let's say you engineer a biosensor that is $99\%$ sensitive and $99\%$ specific.

- You deploy it in a general ward where the prior probability (prevalence) of a specific AMR strain is only $1\%$.

- **The Reality Check:** If the alarm rings, what is the probability the patient actually has the resistant bug?
  - It's only $50\%$. Half of your alarms will be false positives.

- **The Takeaway:** Engineering a highly accurate device is not enough. If you deploy it in the wrong clinical context (the wrong prior), it will cause chaos, over-prescription of antibiotics, and ultimately fail HTA evaluation.
...
- **The Challenge:** We cannot wait 10 years for a clinical trial to prove a new technology reduces AMR at the population level.

- **Mechanistic Models (Transmission Dynamics):** - Adapting SIR (Susceptible-Infected-Recovered) models to hospital wards.
  - Simulating how a rapid diagnostic cuts the chain of transmission.

- **Decision-Analytic Modelling:** - Using decision trees and Markov models.
  - Mapping the probabilistic patient pathways with vs. without your new technology.
